...
## High-level summary
- Although the classic, elegantly-written paper has been publised for nearly 10 years, it still enjoys high-citation until now as it's one of the first papers introducing latent factor algorithm which is especially tailored to implicit feedback recommenders.
- It also provides scalable optimization procsdure which is claimed to be scaled **_linearly_** with the data size.
-  Surprisingly, the proposed algorithm allows explaining the recommendation results to the end users, which is a rarity among the latent factor models. This interesting feature would probably play a big role in integrating the AI solution into some traiditonal enterprise clients.
...
### Neighborhood models
Let's retrict the discussion into item-based neighborhood models as it has better scalability and improved accuracy. Our goal is to compute the similarity of item $i$ and item $j$, uaually based on Pearson correlation coeffcient $s_{i,j}$ and then predict $r_{u,i} , u \in U, i \in I$.
...
M. Deshpande, G. Karypis, “Item-based top-N recom- mendation algorithms”, _ACM Trans. Inf. Syst. 22 (2004) 143-177._
...
$$
Similarly, $p_{ui}$ = $\textbf{x}_{u}^{\intercal} \textbf{y}_{i}$ where $\textbf{x}_{u}$ as the user-factors and $\textbf{y}_{i}$ as the item-factors.
...
- They adjusted the _momentum effect_ by for the $t$-th show after a channel tune, we assign it aassigning it a weighting $\frac{e^{-(2t-6)}}{1+e^{-(2t-6)}}$.
### Evaluation
#### Notations
